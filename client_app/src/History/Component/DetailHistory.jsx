    ...
    return (
        <div>
            <div className="container" style={{ paddingTop: '3rem' }}>
                ...
                <ul>
                    <li style={{ fontSize: '1.1rem' }}>Mã hóa đơn: <span>{order._id}</span></li>
                    <li style={{ fontSize: '1.1rem' }}>Số điện thoại: <span>{order.id_note && order.id_note.phone}</span></li>
                    <li style={{ fontSize: '1.1rem' }}>Họ tên khách: <span>{order.id_note && order.id_note.fullname}</span></li>
                    <li style={{ fontSize: '1.1rem' }}>Tổng tiền: <span>{new Intl.NumberFormat('vi-VN',{style: 'decimal',decimal: 'VND'}).format(order.total) + ' VNĐ'}</span></li>
                    <li style={{ fontSize: '1.1rem' }}>Tiền ship: <span>{new Intl.NumberFormat('vi-VN',{style: 'decimal',decimal: 'VND'}).format(order.feeship) + ' VNĐ'}</span></li>
                    <li style={{ fontSize: '1.1rem' }}>Phương thức thanh toán: <span>{order.id_payment ? order.id_payment.pay_name : 'Chưa có thông tin'}</span></li>
                    <li style={{ fontSize: '1.1rem' }}>Trạng thái: <span style={{ color: order.pay ? 'green' : 'red' }}>{order.pay ? 'Đã thanh toán' : 'Chưa thanh toán'}</span></li>
                </ul>
                            ...
                            <form action="#">
                                <div className="table-content table-responsive">
                                    <table className="table">
                                        <thead>
                                            <tr>
                                                <th className="li-product-remove">Hình sản phẩm</th>
                                                <th className="li-product-thumbnail">Tên sản phẩm</th>
                                                <th className="cart-product-name">Giá tiền sản phẩm</th>
                                                    <th className="li-product-price">Số lượng</th>
                                                <th className="li-product-price">Kích cỡ</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {
